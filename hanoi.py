...
def move(n, source, target, source_name, target_name):
    global move_count
    move_count += 1
    progress = (move_count / total_moves) * 100

    print(f"Move {move_count}: Move disk {n} from {source_name} to {target_name}")
    print(f"Move {move_count}: Move disk {n} from {source} to {target}")
